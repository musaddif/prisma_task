/**
 * Uproot QuickClean™ Gloves Pro — complete product data
 * Source: https://uprootclean.com/products/uproot-grooming-gloves
 * Isolated layout: pdpLayout "grooming-gloves" → GroomingGlovesPdpSections
 */

export const groomingGloves = {
  id: 3,
  handle: "uproot-grooming-gloves",
  name: "Uproot QuickClean™ Gloves Pro",
  category: "Pet Grooming",
  price: 29.99,
  originalPrice: 29.99,
  compareAtPrice: 29.99,
  rating: 4.9,
  reviews: 68,
  inStock: false,
  badge: "New",
  subtitle:
    "Gentle, hypoallergenic grooming gloves that cleanse, deodorize, and condition without water.",
  description:
    "The Uproot Grooming Gloves Pro are crafted with pet-safe, non-toxic ingredients to ensure a soothing and effective grooming experience. The formula is free from harsh chemicals, prioritizing your pet's comfort and health during every use. Nourishing ingredients like shea butter and jojoba oil deeply condition the skin and coat, while vitamin E supports a healthy, shiny finish. With a pH-balanced, hypoallergenic composition, these gloves provide gentle care that leaves your pet feeling refreshed and rejuvenated.",
  pdpStyle: "reference",
  pdpLayout: "grooming-gloves",
  sourceUrl: "https://uprootclean.com/products/uproot-grooming-gloves",

  images: {
    main: "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/1_4afebac8-911e-4f32-9d63-8046521798bd.jpg?v=1762200584",
    gallery: [
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/2_bb19ce89-a16a-4076-9d15-37dddfbd6fbe.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/3_87fbed61-6ca4-4513-8651-503e62350cb5.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/8_47754bfe-a9a3-4e64-a934-df0325be94a5.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/4_1b01a373-63ea-4bfd-9bd9-7725c722a9d4.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/5_35617328-1525-4b90-8f8c-57c0ebb58bff.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/6_dfaa225d-8cb6-4903-ba7f-a100cefc4f39.jpg?v=1734528038",
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/2_62b53013-010b-48d9-a67f-b16da83d2bc8.jpg?v=1734528033",
    ],
  },

  optionAxes: [
    {
      name: "Size",
      key: "size",
      values: ["Single Pack", "Double Pack", "Four Pack"],
    },
  ],

  variants: [
    {
      id: 47469406453916,
      title: "Single Pack",
      price: 29.99,
      compareAtPrice: 29.99,
      sku: "up-gloves-20pk-1-shop",
      available: false,
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/1_4afebac8-911e-4f32-9d63-8046521798bd.jpg?v=1762200584",
      displayName: "Uproot QuickClean™ Gloves Pro",
      size: "Single Pack",
    },
    {
      id: 47469406486684,
      title: "Double Pack",
      price: 59.99,
      compareAtPrice: 59.99,
      sku: "up-gloves-20pk-2-shop",
      available: false,
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/1_4afebac8-911e-4f32-9d63-8046521798bd.jpg?v=1762200584",
      displayName: "Uproot QuickClean™ Gloves Pro",
      size: "Double Pack",
      badge: "Most Popular",
    },
    {
      id: 47469406519452,
      title: "Four Pack",
      price: 119.99,
      compareAtPrice: 119.99,
      sku: "up-gloves-20pk-4-shop",
      available: false,
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/1_4afebac8-911e-4f32-9d63-8046521798bd.jpg?v=1762200584",
      displayName: "Uproot QuickClean™ Gloves Pro",
      size: "Four Pack",
      badge: "Buy 2 Get 2 Free",
    },
  ],

  benefits: [
    "Gentle, Hypoallergenic, and Lickable Formula",
    "Effortlessly Removes Shedding Hair & Dirt",
    "Cleans and Conditions Pet Coat (Dry Bath)",
    "Comfortable Fit for Mess-Free Grooming",
    "20 Gloves in 1 Bag",
  ],

  ingredients: [
    "Aqua/Water/Eau: Provides a gentle, hydrating base for the formula.",
    "Propylene Glycol: Moisturizes the coat and prevents dryness.",
    "Caprylyl Glycol: Hydrates skin and has antimicrobial properties.",
    "Glycerin: Softens and conditions the coat for a silky finish.",
    "Citric Acid: Balances pH to ensure skin health and comfort.",
    "Caprylhydroxamic Acid: Natural preservative for product stability.",
    "Caprylyl/Capryl Glucoside: Mild cleanser that removes dirt and excess oils.",
    "Sodium Benzoate: Safe preservative to maintain freshness.",
    "Tetrasodium Glutamate Diacetate: Enhances preservative effectiveness and product stability.",
    "Butyrospermum Parkii (Shea Butter): Deeply nourishes and soothes the skin.",
    "Tocopherol: Antioxidant that promotes healthy skin and coat.",
    "Simmondsia Chinensis (Jojoba) Seed Oil: Conditions and protects the coat while reducing irritation.",
  ],

  productDetailsNote: "Size: 20 Glove Wipes Pro · Weight: 9oz",

  trustBadges: [
    {
      label: "Fast Shipping & Easy Returns",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/usp-shipping_9cc5bef4-150e-45f4-9126-66e9f45ab7fa.png?v=1677850758",
    },
    {
      label: "60 day Money Back Guarantee",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/usp-money.png?v=1677848055",
    },
    {
      label: "Ships From Florida",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/usp-flag_8ddeed8f-8981-45b3-9672-3933ce627167.png?v=1718953949",
    },
    {
      label: "Fast Customer Support",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/usp-support.png?v=1677848449",
    },
  ],

  promoGift: {
    label: "Mystery Gift",
    value: "$10 VALUE",
    note: "Order by End of Day for Guaranteed Free Gifts",
    icon: "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/ico-mistery-gift-10.png?v=1762979274",
  },

  videos: [],

  specs: {
    type: "Disposable grooming glove wipes",
    count: "20 gloves per bag",
    weight: "9 oz",
    formula: "Hypoallergenic, lickable, pH-balanced, alcohol-free",
    features: "Cleanses, deodorizes, conditions; dry bath between washes",
    uses: "After walks, muddy days, between baths, on-the-go travel",
    safety: "Safe if licked; gentle on sensitive eyes and ears",
  },

  storySections: [
    {
      id: "brushing-myth",
      title: "Think Brushing Keeps Your Dog Clean? Think Again",
      imageSide: "left",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/2_bb19ce89-a16a-4076-9d15-37dddfbd6fbe.jpg?v=1734528038",
      paragraphs: [
        "Every walk through the park, every roll in the grass, every snuggle on the couch - your dog's fur is picking up a microscopic world of dirt, bacteria, and allergens you can't see.",
        "And here's the scary part: brushing alone doesn't fix it.",
        "Over time, this invisible buildup turns your pet's coat into a hotbed for itching, irritation, and painful hot spots. What looks like harmless scratching could actually be the beginning of allergic dermatitis - the most common allergic reaction in dogs, according to the American Kennel Club.",
        "Flea saliva, food sensitivities, or even environmental triggers like pollen can cause severe itching that only gets worse with scratching.",
      ],
    },
    {
      id: "protects-skin",
      title: "This Stops Infections By Protecting Your Dog's Skin & Coat",
      imageSide: "right",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/3_87fbed61-6ca4-4513-8651-503e62350cb5.jpg?v=1734528038",
      paragraphs: [
        "These aren't just your average grooming tools. Uproot's Grooming Gloves are a game-changer, combining cleaning, deodorizing, and conditioning into one easy-to-use product that goes deeper than ever before.",
        "Here's How They Work:",
        "The glove design lets you clean your dog like you're just petting them, creating a stress-free grooming experience for both you and your pet. They also give you control to reach tricky spots like under the legs, around the tail, and even sensitive areas.",
        "But it's not just about surface-level cleaning. Uproot's specially formulated gloves lift away dirt, allergens, and bacteria that brushing alone can't reach. And they don't stop there. They're made with healthy ingredients that are good for your pets.",
      ],
    },
    {
      id: "gentle-care",
      title: "Gentle Care for a Healthy Coat",
      imageSide: "left",
      image:
        "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/8_47754bfe-a9a3-4e64-a934-df0325be94a5.jpg?v=1734528038",
      paragraphs: [
        "The Uproot Grooming Gloves Pro are crafted with pet-safe, non-toxic ingredients to ensure a soothing and effective grooming experience. The formula is free from harsh chemicals, prioritizing your pet's comfort and health during every use. Nourishing ingredients like shea butter and jojoba oil deeply condition the skin and coat, while vitamin E supports a healthy, shiny finish. With a pH-balanced, hypoallergenic composition, these gloves provide gentle care that leaves your pet feeling refreshed and rejuvenated.",
      ],
    },
  ],

  howItWorks: {
    title: "Here's How They Work",
    description:
      "The glove design lets you clean your dog like you're just petting them, creating a stress-free grooming experience for both you and your pet.",
    image:
      "https://cdn.shopify.com/s/files/1/0551/2957/1484/files/4_1b01a373-63ea-4bfd-9bd9-7725c722a9d4.jpg?v=1734528038",
    steps: [
      {
        title: "Slip On the Glove",
        description:
          "Put on a QuickClean glove for a comfortable, mess-free fit.",
      },
      {
        title: "Pet & Clean",
        description:
          "Wipe down your dog like you're petting them — reach under the legs, around the tail, and sensitive areas.",
      },
      {
        title: "Lift Dirt & Allergens",
        description:
          "The formula lifts away dirt, allergens, and bacteria that brushing alone can't reach.",
      },
      {
        title: "Condition & Dispose",
        description:
          "Shea butter, jojoba oil, and vitamin E condition the coat. Toss the glove when finished.",
      },
    ],
  },

  faqs: [
    {
      question: "Is this product really as good as advertised?",
      answer:
        "Yes! All Uproot Clean products are backed by our 60-Day Money Back Guarantee. If you are not 100% satisfied, simply contact us for a full refund.",
    },
    {
      question: "How long does delivery take?",
      answer:
        "Orders are shipped from our US warehouse and typically arrive in 3-5 business days. Free shipping applies on orders over $49.",
    },
    {
      question: "What is the return policy?",
      answer:
        "We offer an iron-clad 60-day satisfaction guarantee. Try it in your home risk-free.",
    },
    {
      question: "Are the gloves safe if my pet licks them?",
      answer:
        "Yes. The formula is lickable, hypoallergenic, and made with pet-safe ingredients including shea butter, jojoba oil, and vitamin E.",
    },
    {
      question: "How many gloves are in each pack?",
      answer: "Each Single Pack includes 20 Glove Wipes Pro (9oz).",
    },
  ],

  reviewsList: [
    {
      id: "gg-barbara-m",
      rating: 4,
      title: "Nice idea!",
      body: "I love these gloves, my dogs tolerate them. Lol They are great for massaging their coats before I brush and they are deodorizing but not overly scented. Company is great to communicate with . Customer service was on point!",
      author: "Barbara M.",
      verified: true,
      date: "2026-05-04",
    },
    {
      id: "gg-debra-a",
      rating: 5,
      title: "Quick Clean Gloves",
      body: "I absolutely love the glove shaped wipes to use on my Border Collie's paws and legs when coming in from outside, especially lately with all the rain we have had. Wet muddy paws are not something I want tracked into our floors, rugs, or furniture! So thank you for a great and convenient product to have on hand. No pun intended!!",
      author: "Debra A.",
      verified: true,
      date: "2026-05-04",
    },
    {
      id: "gg-john-r",
      rating: 5,
      title: "EZ Clean Your Pup!",
      body: "Bathtime for pets is often more like bath battle. And nobody likes a stinky dog, except maybe another stinky dog. Pop these babies on, and trick them into thinking they are getting a massage. Then toss 'em out, towel off or let 'em air dry, and ahhhhhhhhh....100% less stinky, and 100% more shiny!",
      author: "John R.",
      verified: true,
      date: "2025-12-04",
    },
    {
      id: "gg-dawn-e",
      rating: 5,
      title: "These help with my dirt devil",
      body: "We live in a dry dusty area and our dogs get filthy multiple times a day. In between showers I am constantly wiping them down w wet rags. These gloves are nice because it's more like petting the dog then wiping them down with a wet rag and they tolerate it better.",
      author: "Dawn E.",
      verified: true,
      date: "2025-09-04",
    },
    {
      id: "gg-patricia-c",
      rating: 5,
      title: "awesome",
      body: "My dog usually hates grooming! These grooming gloves are a lifesaver for quick cleanups.",
      author: "Patricia C.",
      verified: true,
      date: "2025-09-04",
    },
  ],

  reviewSummary: {
    rating: 4.9,
    count: 68,
    recommendPercent: 100,
    breakdown: [
      { stars: 5, count: 61 },
      { stars: 4, count: 7 },
      { stars: 3, count: 0 },
      { stars: 2, count: 0 },
      { stars: 1, count: 0 },
    ],
  },

  guarantee: "60 day Money Back Guarantee",

  washerTypes: null,
  featureCards: null,
  comparisonTable: null,
  bundles: [],
  pressMentions: [],
  designedToClean: [],
  functions: [],
  quietTechnology: null,
};

export default groomingGloves;
